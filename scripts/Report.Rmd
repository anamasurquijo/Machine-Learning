...
- **Response variables**: Total rentals (cnt), casual users, registered users

All continuous weather variables are normalized (0-1 scale).
...
bike <- bike_data %>%
  mutate(
    ...
    log_cnt = log(cnt),  
    high_demand = factor(ifelse(cnt > median(cnt), 1, 0), 
                        levels = c(0, 1), labels = c("Low", "High")),
    reg_percentage = registered / (cnt)
  )
...
bike %>%
  summarise(
    ...
    Total_Rentals = c(mean(cnt), median(cnt), sd(cnt), min(cnt), max(cnt))
  )
```
...
p2 <- ggplot(bike, aes(x = season, y = cnt, fill = season)) +
...
**Key Insights**: Clear morning (8am) and evening (5-6pm) commuter peaks. Fall shows highest demand, while Spring has lowest. This suggests targeting bike availability at stations near business districts during rush hours.

## Weather Effects

```{r weather-plots}
p3 <- ggplot(bike, aes(x = temp, y = cnt)) +
...
p4 <- ggplot(bike, aes(x = weathersit, y = cnt, fill = weathersit)) +
  geom_boxplot(alpha = 0.7) +
...
**Key Insights**: Strong positive relationship between temperature and demand (up to ~0.7 normalized temp), then slight decline. Clear weather drives highest demand; light rain reduces rentals by ~40%. This informs dynamic pricing strategies during adverse weather.
...
4. **Stratified Sampling**: We use `createDataPartition()` to maintain the same high/low demand ratio in both sets, preventing train-test distribution mismatch.
...
if (file.exists("../models/glm_poisson.rds")) {
  ...
  glm_poisson <- glm(casual ~ season + yr + hr + holiday + weekday + 
...
# Key coefficients
summary(glm_poisson)$coefficients[1:8, ] %>%
  kable(digits = 3, caption = "Poisson GLM Coefficients (First 8)")
...
cat("RMSE:", round(glm_pois_rmse, 2), "\n")
```

**Interpretation**: Holidays dramatically increase casual users (exp(coef) ≈ 1.9x), while working days reduce them. Midday hours (10am-3pm) show peak casual usage, contrasting with registered users' commuter pattern.

**Business Value**: Target marketing campaigns for tourists/leisure riders during holidays and weekends at scenic locations.

## GLM Binomial - User Composition Analysis
**Lead Analyst: Student3_Lead**
...
# Prepare data with numeric predictors only
train_nn <- train %>%
  mutate(
    season_num = as.numeric(season),
    yr_num = as.numeric(yr),
    ...
    workingday_num = as.numeric(workingday) - 1,
    weathersit_num = as.numeric(weathersit),
...
if (file.exists("../models/nn_model.rds")) {
  ...
  nn_model <- nnet(high_demand_num ~ season_num + yr_num + hr_num + 
                     holiday_num + workingday_num + weathersit_num +
                     temp + hum + windspeed,
                   data = train_nn,
                   size = 8,
                   maxit = 200,
...
nn_accuracy <- mean(nn_pred_class == test$high_demand)
nn_conf <- table(Predicted = nn_pred_class, Actual = test$high_demand)

cat("\nNeural Network Performance:\n")
cat("Accuracy:", round(nn_accuracy * 100, 1), "%\n")
print(nn_conf)
```

**Interpretation**: Neural network achieves strong classification accuracy by learning complex interactions between hour, weather, and temporal features. High recall for high-demand periods ensures sufficient bike availability.

**Business Value**: Triggers alerts for high-demand shifts, enabling proactive bike redistribution.
