...
kable(
  glm_binomial_coefs,
  digits  = 3,
  caption = "Binomial GLM – Probability that a rider is registered (Mayra)"
)
```

### Interpretation

The binomial GLM models how calendar and weather conditions change the **likelihood that a ride comes from a registered user**.

- **Workingday**: Strong positive effect → registered users dominate on working days, matching commuter-driven demand.
- **Hour of day**: Commute hours (7–9am, 4–6pm) increase registered-user share; midday hours attract more casual riders.
- **Weather**: Bad weather increases the proportion of registered riders (casual riders drop off more).
- **Temperature**: Warmer hours slightly reduce the registered share → warm weather attracts more leisure/casual users.
- **Season**: Winter shows higher registered-user share; summer shows more casual-user activity.

Overall, the model achieves **high accuracy** at classifying whether an hour is dominated by registered vs casual users.

### Business Value

This model identifies **WHO uses the service under different conditions**:

- **Commuter core**: Registered users dominate working days and commute hours → commercial/corporate subscription opportunities.
- **Tourist/leisure windows**: Midday, warm, dry conditions attract casual riders → ideal for short-term promotions and onboarding campaigns.
- **Operational planning**: Redistribute bikes differently based on rider type dominance.
- **Marketing alignment**: Tailor messaging to “commuters vs leisure riders” depending on season/hour/weather.

This model supports membership strategy, pricing, and targeted campaigns.
...
The GAM reveals **nonlinear patterns** in demand:

- **Temperature (smooth)**: Demand increases sharply from cold to mild temperatures, then plateaus at high temperatures.
- **Perceived temperature (atemp)**: Similar nonlinear rise, confirming strong comfort-weather effects.
- **Humidity**: Moderate humidity supports demand; extremely high humidity reduces casual-bike usage.
- **Windspeed**: Light wind has minimal effect; stronger wind reduces demand nonlinearly.
- **Hour**: Smooth cyclical pattern with two clear peaks (commute times).

GAM’s flexibility shows that demand doesn’t react linearly: passengers respond strongly to *transitions* in weather (e.g., cold-to-mild jump).

### Business Value

The GAM uncovers subtle patterns useful for forecasting and operations:

- **Weather-sensitive planning**: Shows exactly when weather transitions cause demand spikes or drops.
- **Hourly redistribution optimization**: Identifies peak and shoulder hours with nonlinear accuracy.
- **Season-aware marketing**: Marketing teams can use temperature-dependent demand curves to time leisure promotions.
- **Infrastructure decisions**: Helps allocate bikes during heatwaves, heavy humidity, or strong winds.

This model adds predictive power and enhances decisions involving **weather**, **seasonality**, and **hour-of-day** dynamics.
