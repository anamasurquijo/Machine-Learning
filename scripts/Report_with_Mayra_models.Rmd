...
1. **Strong and important predictors**  
   - **Temperature (`temp`, ~0.63)** and **feeling temperature (`atemp`, ~0.63)** show strong positive correlations.  
...
## Generalized Linear Models – Binomial – Mayra

We model the proportion of registered users using a binomial Generalized Linear Model (GLM). The response is defined as the number of registered users out of the total number of users in each hour. This model helps identify how season, time of day, holidays, working days, weather situation, temperature, and humidity influence the likelihood that a rider is a registered user.

```{r glm-binomial, cache=TRUE, dependson="train-test-split", message=FALSE, warning=FALSE}
...
if (file.exists("../models/glm_binomial.rds")) {
  ...
  glm_binomial <- glm(
    cbind(reg_count, total_count - reg_count) ~
      season + hour + holiday + workingday +
...
kable(
  glm_binomial_coefs,
  digits  = 3,
  caption = "Binomial GLM for Registered User Proportion – Coefficients (Mayra)"
)
```

The coefficients show how each predictor changes the log-odds of a ride being made by a registered user. Positive coefficients increase this probability, while negative coefficients decrease it.


## Generalized Additive Model (GAM) – Mayra

To capture nonlinear relationships between weather conditions and bike demand, we use a Generalized Additive Model (GAM). Smooth functions allow temperature, perceived temperature, humidity, and wind speed to vary flexibly, while seasonal, hourly, holiday, and working-day effects remain linear.

```{r gam-model, cache=TRUE, dependson="train-test-split", message=FALSE, warning=FALSE}
...
if (file.exists("../models/gam_model.rds")) {
  ...
  gam_model <- gam(
    count ~
      ...
      s(humidity) +
      s(windspeed) +
      season + hour + holiday + workingday + weather,
    data   = train,
    method = "REML"
  )
...
```{r gam-plots, fig.height=6, fig.width=10, message=FALSE, warning=FALSE}
# Visualising smooth effects of continuous predictors
par(mfrow = c(2, 2))
plot(gam_model, shade = TRUE, seWithMean = TRUE, pages = 1)
par(mfrow = c(1, 1))
```

The GAM reveals nonlinear demand patterns such as optimal temperature ranges, reduced demand at very high humidity, and drops in ridership under strong wind, providing more flexibility than linear models.
